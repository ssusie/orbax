...
def memory_per_device_gb():
  ...
  #   )
  memory_per_chip = sum(
    [x.addressable_shards[0].data.size * x.addressable_shards[0].data.itemsize for x in jax.live_arrays()]
    )
  #  arr.size * arr.itemsize
  # memory_per_chip = bytes_per_param * sum(
  #   [np.prod(x.addressable_shards[0].data.shape) for x in
  #    list({arr.unsafe_buffer_pointer(): arr for arr in jax.live_arrays()}.values())
  #   ]
  #   )
  # memory_per_chip = bytes_per_param * sum(
  #   [np.prod(x.shape) for x in
  #    list({arr.addressable_shards[0].data.unsafe_buffer_pointer(): arr.addressable_shards[0].data for arr in jax.live_arrays()}.values())
  #   ]
  #   )
  # memory_per_chip = 0
